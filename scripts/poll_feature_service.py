...
LANDING_FLIGHT_COLUMNS = {'landing_operator': 'operator_code',
...
def get_ticket(group, connection_json):
    ...
    engine = create_engine('postgresql://{username}:{password}@{ip_address}:{port}/{db_name}'
                           .format(**connection_info[submission_type]))
...
def poll_feature_service(log_dir, download_dir, agol_credentials_json, email_credentials_json, db_credentials_json, ssl_cert, landings_conn, tracks_conn):
    ...
    with open(db_credentials_json) as j:
        connection_dict = json.load(j)

    submissions = submissions.groupby(['submitter', 'submission_type'])\
        .apply(lambda g: get_ticket(g, db_credentials_json))

    # Process and import the landing data. The flight tracks will need to be unzipped, but the track data need to be validated/edited before they can be imported. This is handled manually via a separate web app
    ...
    with engine.connect() as conn, conn.begin():
        ...
        for table_name, data_types in field_types.items():
            ...
            for field, _ in data_types.loc[data_types == 'esriFieldTypeDate'].iteritems():
                df[field] = df[field].apply(julian_date_to_datetime).apply(pd.Timestamp)
        ...
        '''landing_flights = pd.read_sql("SELECT * FROM %s WHERE submission_type = 'landings'" % main_table_name, conn)\
            .dropna(axis=1, how='all') # columns for all track submissions will be empty so drop them
        track_flights = pd.read_sql("SELECT * FROM %s WHERE submission_type = 'tracks'" % main_table_name, conn)\
        ...
        '''track_flights['ticket'] = track_flights.merge(submissions, on='globalid').ticket
        track_flights.to_sql()

        landing_flights['ticket'] = landing_flights.merge(submissions, on='globalid').ticket'''
    ...
    #   calculate fees
    fees = passengers.groupby('parentglobalid').sum() * LANDING_FEE
    passengers.landing_type = passengers.landing_type.apply(lambda x: x.split('_')[-1])
    #   make flight IDs and attach to landings and fees
    landing_flights.rename(columns=LANDING_FLIGHT_COLUMNS, inplace=True)
    landing_flights['flight_id'] = landing_flights.registration + '_' + landing_flights.departure_datetime.dt.floor('15min').dt.strftime()
...
def main(log_dir, download_dir, agol_credentials_json, email_credentials_json, db_credentials_json, ssl_cert):
    ...
    '''
    #try:
    # If I wrap this in a with engine.connect() as conn, conn.begin() block and pass, conn as an arg, then if I insert a submission record and the process fails before I can insert the flight data, the entire transaction will be rolled back
    poll_feature_service(log_dir, download_dir, agol_credentials_json, email_credentials_json, db_credentials_json, ssl_cert)
